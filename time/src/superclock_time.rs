...
impl SuperclockTime {
    ...
    pub fn from_seconds_f64_lossy(seconds: SecondsF64) -> Self {
        if seconds.0 > 0.0 {
    ...
    /// [`SuperclockTime`]: struct.SuperclockTime.html
    /// [`SecondsF64`]: struct.SecondsF64.html
    pub fn from_seconds_f64_floor_lossy(seconds: SecondsF64) -> Self {
        if seconds.0 > 0.0 {
            let mut secs = seconds.0.trunc() as u32;
            let mut ticks =
    ...
    /// [`SuperclockTime`]: struct.SuperclockTime.html
    /// [`SecondsF64`]: struct.SecondsF64.html
    pub fn from_seconds_f64_ceil_lossy(seconds: SecondsF64) -> Self {
        if seconds.0 > 0.0 {
            let mut secs = seconds.0.trunc() as u32;
            let mut ticks =
    ...
    pub fn from_seconds_f64_with_sub_tick_lossy(seconds: SecondsF64) -> (Self, f64) {
        if seconds.0 > 0.0 {
            let mut secs = seconds.0.trunc() as u32;
    ...
    /// [`SuperclockTime`]: struct.SuperclockTime.html
    /// [`FrameTime`]: struct.FrameTime.html
    pub fn from_frame_maybe_lossy(sample: FrameTime, sample_rate: SampleRate) -> Self {
        match sample_rate.0 as usize {
            ...
            _ => Self::from_seconds_f64_lossy(SecondsF64(sample.0 as f64 / sample_rate.as_f64())),
        }
    }
    ...
    ///
    /// [`SecondsF64`]: struct.SecondsF64.html
    pub fn to_seconds_f64_lossy(&self) -> SecondsF64 {
        SecondsF64(
    ...
    pub fn to_musical_lossy(&self, bpm: f64) -> MusicalTime {
        self.to_seconds_f64_lossy().to_musical_lossy(bpm)
    }
