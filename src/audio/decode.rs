...
pub(crate) fn decode_resampled(
    ...
    let mut tmp_conversion_buf: Option<SymphoniaAudioBuffer<f32>> = None;
        ...
        match decoder.decode(&packet) {
            Ok(decoded) => {
                ...
                if tmp_conversion_buf.is_none() {
                    ...
                    tmp_conversion_buf = Some(SymphoniaAudioBuffer::new(duration as u64, spec));
                }
                ...
                if tmp_conversion_buf.capacity() < decoded.capacity() {
                    ...
                    *tmp_conversion_buf = SymphoniaAudioBuffer::new(duration as u64, spec);
                }
...
pub(crate) fn decode_f32(
    ...
    //     })
    //     .collect();
    let mut final_buf = Write::new(Dynamic::with_topology(
        num_channels.get(),
        estimated_final_frames,
    ));
    ...
    while let Ok(packet) = probed.format.next_packet() {
        ...
        match decoder.decode(&packet) {
            Ok(decoded) => {
                ...
                let a = audio::wrap::dynamic(converted_planes);

                audio::io::copy_remaining(audio::wrap::dynamic(converted_planes), final_buf);

                // for (final_ch, decoded_ch) in final_buf.iter_mut().zip(converted_planes) {
                //     audio::io::copy_remaining(decoded_ch, final_ch);
    ...
    Ok(AudioBuffer::new(final_buf, sample_rate.into()))
}
...
fn decode_u8_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<u8>>,
    num_channels: NonZeroUsize,
) {
...
fn decode_u16_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<u16>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        decoded_channels[i].extend_from_slice(packet.chan(i));
...
fn decode_u24_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<u24>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
...
fn decode_u32_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<u32>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        for s in packet.chan(i).iter() {
...
fn decode_i8_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<i8>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        decoded_channels[i].extend_from_slice(packet.chan(i));
...
fn decode_i16_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<i16>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        decoded_channels[i].extend_from_slice(packet.chan(i));
...
fn decode_i24_packet(
    decoded_channels: &mut Vec<Vec<[u8; 3]>>,
    packet: Cow<SymphoniaAudioBuffer<i24>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        for s in packet.chan(i).iter() {
...
fn decode_i32_packet(
    decoded_channels: &mut Vec<Vec<f32>>,
    packet: Cow<SymphoniaAudioBuffer<i32>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        for s in packet.chan(i).iter() {
...
fn decode_f32_packet(
    decoded_channels: &mut Vec<Vec<f32>>,
    packet: Cow<SymphoniaAudioBuffer<f32>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        decoded_channels[i].extend_from_slice(packet.chan(i));
...
fn decode_f64_packet(
    ...
    packet: Cow<SymphoniaAudioBuffer<f64>>,
    num_channels: NonZeroUsize,
) {
    for i in 0..num_channels.get() {
        decoded_channels[i].extend_from_slice(packet.chan(i));
